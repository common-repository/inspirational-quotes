<?php
/*
Plugin Name: Inspirational Quotes
Plugin URI: http://www.blogandrew.com/inspiration-widget
Description: This simple WordPress plugin displays inspirational quotes in your blog's sidebar. To install, click activate and then go to Appearance > Widgets to find 'Inspirational Quotes'. All settings are contained within the widget. 
Version: 1.0
Author: Andrew Olson
Author URI: http://www.blogandrew.com
*/

add_action("widgets_init", array('Inspirational_Quotes', 'register'));
register_activation_hook( __FILE__, array('Inspirational_Quotes', 'activate'));
register_deactivation_hook( __FILE__, array('Inspirational_Quotes', 'deactivate'));

class Inspirational_Quotes {
  function activate(){
    $data = array('title' => 'Inspirational Quote', 'update' => 'Daily', 'credit' => 'Yes');
    if ( ! get_option('inspirational_quotes')){
      add_option('inspirational_quotes' , $data);
    } else {
      update_option('inspirational_quotes' , $data);
    }
  }
  
 function control(){
  $data = get_option('inspirational_quotes');
  ?>
    <p><label>Widget Title: <input name="inspirational_quotes_title"
type="text" value="<?php echo $data['title']; ?>" /></label></p>
<p><label>Update Quotes:</label> 
  <select name="inspirational_quotes_update">
...
<p><label>Give Us Credit? </label>
  <select name="inspirational_quotes_credit">
...
<b>Random:</b> Updates quote on refresh<br/>
</p>

  <?php
   if (isset($_POST['inspirational_quotes_title'])){
   	$data['title'] = attribute_escape($_POST['inspirational_quotes_title']);
    $data['update'] = attribute_escape($_POST['inspirational_quotes_update']);
    $data['credit'] = attribute_escape($_POST['inspirational_quotes_credit']);
    update_option('inspirational_quotes', $data);
  }
}

  function deactivate(){
    delete_option('inspirational_quotes');
  }
...
if ($data['update'] == "Daily"){
$thelink = "Andrew Olson";
} else {
$thelink = "Andrew Olson";
}
		
if ($data['credit'] == "Yes"){
echo 'Plugin by <a href="http://www.blogandrew.com/">'.$thelink.'</a>';} else {}

echo $args['after_widget'];
  }
  
function register(){
    register_sidebar_widget('Inspirational Quotes', array('Inspirational_Quotes', 'widget'));
    register_widget_control('Inspirational Quotes', array('Inspirational_Quotes', 'control'));
  }
}
